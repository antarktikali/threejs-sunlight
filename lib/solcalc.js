...
function getTimeLocal( date_ = new Date() )
{
...
function getAzEl( lat_, lon_,  date_ = new Date() ) {
	var jday = getJD( date_ );
	var tl = getTimeLocal( date_ );
	var tz = date_.getTimezoneOffset() / -60;
	var dst = true;
	var total = jday + tl/1440.0 - tz/24.0;
	var T = calcTimeJulianCent( total );
	sunOrientation = calcAzEl( false, T, tl, lat_, lon_, tz );
